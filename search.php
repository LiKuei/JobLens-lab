...
// 4. Fetch Sub-data for JavaScript Arrays
// Recruitment Jobs
$stmt = $pdo->prepare("SELECT Id, Name, Url, Salary FROM recruitment WHERE CompanyId = ?");
$stmt->execute([$company['Id']]);
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full group">
                    ...
                    <div class="pt-4 border-t border-slate-100 mt-auto">
                        <div class="grid grid-cols-2 gap-2">
                            <a href="${job.Url}" target="_blank" class="text-center py-2.5 bg-cyan-100 text-cyan-700 font-bold rounded-lg hover:bg-cyan-600 hover:text-white transition-colors group-hover:shadow-md">
                                立即應徵 <i class="fa-solid fa-arrow-up-right-from-square ml-1"></i>
                            </a>
                            <a href="salary-negotiation.php?id=<?= (int)$company['Id'] ?>&job=${encodeURIComponent(job.Id)}" class="text-center py-2.5 bg-violet-100 text-violet-700 font-bold rounded-lg hover:bg-violet-600 hover:text-white transition-colors group-hover:shadow-md">
                                面試談薪 <i class="fa-solid fa-handshake-angle ml-1"></i>
                            </a>
                        </div>
                    </div>
                </div>
